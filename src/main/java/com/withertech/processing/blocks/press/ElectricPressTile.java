...
public class ElectricPressTile extends AbstractMachineTileEntity<RecipePressing> implements IAnimatable, IRestorableTileEntity
{
	// Energy constant
	...
	public ElectricPressTile(MachineTier tier)
	{
		super(MachineType.PRESS.getTileEntityType(tier), INVENTORY_SIZE, tier);
	}
	...
	private <E extends TileEntity & IAnimatable> PlayState predicate(AnimationEvent<E> event)
	{
		...
		if ((controller.getCurrentAnimation() != null && controller.getCurrentAnimation().animationName.equals("press.animation.deploy")) || controller.isJustStarting)
		{
			if (!this.deployed)
			{
				controller.setAnimation(new AnimationBuilder().addAnimation("press.animation.deploy", false).addAnimation("press.animation.idle", true));

				return PlayState.CONTINUE;
			}
		}

		if (event.getAnimatable().getBlockState().get(ElectricPressBlock.LIT))
		{
			controller.setAnimation(new AnimationBuilder().addAnimation("press.animation.run", true));
		} else
		{
			controller.setAnimation(new AnimationBuilder().addAnimation("press.animation.idle", true));
		}
		return PlayState.CONTINUE;
	}
	...
	@Nullable
	@Override
	protected RecipePressing getRecipe()
	{
		if (world == null) return null;
		return world.getRecipeManager().getRecipe(ModRecipes.Types.PRESSING, this, world).orElse(null);
	}

	@Override
	protected int getProcessTime(RecipePressing recipe)
	{
		return recipe.getProcessTime();
	}

	@Override
	protected Collection<ItemStack> getProcessResults(RecipePressing recipe)
	{
		return recipe.getResults(this);
	}

	@Override
	protected Collection<ItemStack> getPossibleProcessResult(RecipePressing recipe)
	{
		return recipe.getPossibleResults();
	}
	...
	public boolean isIngredient(ItemStack stack)
	{
		...
		return Minecraft.getInstance().world.getRecipeManager().getRecipesForType(ModRecipes.Types.PRESSING).stream().anyMatch(recipePressing -> recipePressing.getIngredient().test(stack));
	}

	@Override
	protected boolean canMachineRun(RecipePressing recipe)
	{
		return super.canMachineRun(recipe) && recipe.getPress().test(getStackInSlot(1));
	}
	...
	{
		IFormattableTextComponent name = TextUtil.translate("container", this.getMachineTier().getName() + "_press");
	...
	{
		return new ElectricPressContainer(id, playerInventory, this, this.fields);
	}
	...
	public static class Basic extends ElectricPressTile
	{
		public Basic()
		{
	...
	public static class Advanced extends ElectricPressTile
	{
		public Advanced()
		{
	...
	public static class Elite extends ElectricPressTile
	{
		public Elite()
		{
	...
	public static class Ultimate extends ElectricPressTile
	{
		public Ultimate()
		{
