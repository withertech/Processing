...
public final class FluidPipeNet implements IFluidHandler
{
	...
	private FluidPipeNet(IWorldReader world, Set<BlockPos> pipes, FluidStack contents)
	{
		this.world = world;
	...
	static FluidPipeNet buildNetwork(IWorldReader world, BlockPos pos, FluidStack contents)
	{
		Set<BlockPos> pipes = buildPipeSet(world, pos);
		return new FluidPipeNet(world, pipes, contents);
	}
	...
	private static Set<BlockPos> buildPipeSet(IWorldReader world, BlockPos pos, Set<BlockPos> set)
	{
		...
		for (Direction side : Direction.values())
		{
			...
			if (!set.contains(pos1) && world.getTileEntity(pos1) instanceof FluidPipeTile)
			{
				set.add(pos1);
	...
	private Set<Connection> getConnections(IBlockReader world, BlockPos pos)
	{
		...
		for (Direction direction : Direction.values())
		{
			...
			if (te != null && !(te instanceof FluidPipeTile) && te.getCapability(CapabilityFluidHandler.FLUID_HANDLER_CAPABILITY).isPresent() && world.getBlockState(pos).getBlock() != Blocks.AIR)
			{
	...
	{
		private final FluidPipeNet network;
		private final Direction side;
		private final ConnectionType type;
		private final LazyOptional<Connection> lazyOptional;

		Connection(FluidPipeNet network, Direction side, ConnectionType type)
		{
